    ...
    def forward(self, x):
        for module in self.modules_list:
            if isinstance(module, nn.Transformer):
                # For Transformer, we need src and tgt parameters
                # Using x as both source and target for self-attention
                raise NotImplementedError(
                    "Transformer forward method requires src and tgt parameters, "
                    "which are not provided in this implementation.")
            elif isinstance(module, Meg2VecModel):
                # Meg2VecModel expects a sequence length and in_channels
                x = module(x)["last_hidden_state"]
            else:
                x = module(x)
        return x
