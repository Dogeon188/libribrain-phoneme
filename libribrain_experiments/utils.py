...
class LibriBrainSpeechFiltered(torch.utils.data.Dataset):
    """
    Parameters:
        dataset: LibriBrain dataset.
        apply_sensors_speech_mask (bool, optional): If True, applies a fixed sensor mask to the sensor
    ...
    def __init__(self, **kwargs):
        ...
        self.apply_sensors_speech_mask = True
        ...
        self.balanced_indices = list(range(len(self.dataset.samples)))
        # Shuffle the indices
...
class FilteredHoldoutDataset(torch.utils.data.Dataset):
    """
    Parameters:
        dataset: LibriBrain dataset.
        apply_sensors_speech_mask (bool, optional): If True, applies a fixed sensor mask to the sensor
    ...
    def __init__(self, **kwargs):
        self.dataset = LibriBrainCompetitionHoldout(**kwargs)
        self.apply_sensors_speech_mask = True
        # These are the sensors we identified:
        self.sensors_speech_mask = SENSORS_SPEECH_MASK
    ...
    def __getitem__(self, index):
        ...
        return [sensors, self.dataset[index][1]]


class LibriBrainSpeechMerge3(torch.utils.data.Dataset):
    """
    Merges 3 channels at same position into one channel.
    """

    def __init__(self, reduce: str, **kwargs):
        self.dataset = LibriBrainSpeech(**kwargs)
        self.labels_sorted = [0, 1]
        self.channel_means = self.dataset.channel_means
        self.channel_stds = self.dataset.channel_stds
    ...
    def __getitem__(self, index):
        sample = list(self.dataset[index])
        # Merge 3 channels at same position into one channel
        # sample[0].shape is (n_channels, n_samples)
        sample[0] = sample[0].reshape(-1, 3, sample[0].shape[1])
        if self.reduce == "mean":
            sample[0] = sample[0].mean(dim=1)
        elif self.reduce == "max":
            sample[0] = sample[0].max(dim=1).values
        elif self.reduce == "length":  # square root of sum of squares
            sample[0] = torch.sqrt((sample[0] ** 2).sum(dim=1))

        # Get the label from the middle of the sequence
        label_middle_index = sample[1].shape[0] // 2
        sample[1] = sample[1][label_middle_index]
        return sample
...
DATASETS = {
    ...
    "libribrain_speech_filtered": LibriBrainSpeechFiltered,
    "libribrain_speech_merge3": LibriBrainSpeechMerge3,
...
def get_datasets_from_config(data_config):
    ...
    if "val" in datasets_config:
        ...
        # val_dataset = get_dataset_partition_from_config(datasets_config["val"])
    ...
    if "test" in datasets_config:
        ...
        test_dataset = get_dataset_partition_from_config(datasets_config["test"])
        if train_labels_sorted is not None:
            check_labels(
...
def log_results(result, y, preds, logits, output_path, run_name, hpo_config=None, trainer=None):
    ...
    # since ConfusionMatrix is not serializable, we remove it
    if ("val_cm" in result):
        del result["val_cm"]
...
def run_training(train_loader, val_loader, config, n_classes, best_model_metric="val_f1_macro", module=None, best_model_metric_mode="max"):
    ...
    # if trainer_config["enable_model_summary"] is None:
    if ("enable_model_summary" not in trainer_config or trainer_config["enable_model_summary"] is None):
...
def run_validation(val_loader, module, labels, samples_per_class=None, name="val"):
    disp_labels = labels
    module.eval()
    all_preds = []
    all_logits = []
    ...
    result = {
        f"{name}_cm": wandb.plot.confusion_matrix(
            probs=None,
            y_true=all_targets.cpu().numpy(),
            preds=all_preds.cpu().numpy(),
            class_names=disp_labels
        ),
        f"{name}_naive_acc": naive_acc,
        f"{name}_random_acc": random_acc,
        f"{name}_random_bal_acc": random_balanced_acc,
        f"{name}_random_f1_macro": random_f1_macro,
        f"{name}_random_f1_micro": random_f1_micro,
        f"{name}_random_f1_weighted": random_f1_weighted,
        f"{name}_naive_acc": naive_acc,
        f"{name}_naive_bal_acc": naive_balanced_acc,
        f"{name}_naive_f1_macro": naive_f1_macro,
        f"{name}_naive_f1_micro": naive_f1_micro,
        f"{name}_naive_f1_weighted": naive_f1_weighted,
        f"{name}_naive_loss": naive_loss,
        f"{name}_acc": acc_val,
        f"{name}_bal_acc": bal_acc_val,
        f"{name}_f1_macro": f1_macro_val,
        f"{name}_f1_micro": f1_micro_val,
        f"{name}_f1_weighted": f1_weighted_val,
        f"{name}_loss": loss,
        f"{name}_rocauc_macro": rocauc_macro_val,
        f"{name}_rocauc_micro": rocauc_micro_val,
        f"{name}_naive_rocauc_macro": naive_rocauc_macro,
        f"{name}_naive_rocauc_micro": naive_rocauc_micro,
    }
    if (len(disp_labels) == 2):
        ...
        result[f"{name}_jaccard_index"] = jaccard_index_val
        result[f"{name}_naive_jaccard_index"] = jaccard_index_naive
    ...
    for c in classes:
        ...
        result[f"{name}_class_{c}_acc"] = class_acc
        result[f"{name}_class_{c}_f1"] = class_f1
        result[f"{name}_class_{c}_random_acc"] = class_random_acc
        result[f"{name}_class_{c}_random_f1"] = class_random_f1
        result[f"{name}_class_{c}_naive_acc"] = class_naive_acc
        result[f"{name}_class_{c}_naive_bal_acc"] = class_naive_bal_acc
        result[f"{name}_class_{c}_naive_f1"] = class_naive_f1
        result[f"{name}_class_{c}_bal_acc"] = class_bal_acc
        result[f"{name}_class_{c}_rocauc"] = class_rocauc
